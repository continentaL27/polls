package com.conti.polls.service;

import com.conti.polls.exception.BadRequestException;
import com.conti.polls.exception.ResourceNotFoundException;
import com.conti.polls.model.*;
import com.conti.polls.payload.PagedResponse;
import com.conti.polls.payload.PollRequest;
import com.conti.polls.payload.PollResponse;
import com.conti.polls.payload.VoteRequest;
import com.conti.polls.repository.PollRepository;
import com.conti.polls.repository.UserRepository;
import com.conti.polls.repository.VoteRepository;
import com.conti.polls.security.UserPrincipal;
import com.conti.polls.util.AppConstants;
import com.conti.polls.util.ModelMapper;
...
public class PollService {
    ...
    public PagedResponse<PollResponse> getAllPolls(final UserPrincipal currentUser, int page, int size) {
        ...
        return new PagedResponse<>(pollResponses, polls.getNumberOfElements(),
                polls.getSize(), polls.getTotalElements(), polls.getTotalPages(), polls.isLast());
    }
    ...
    public PagedResponse<PollResponse> getPollsCreatedBy(String username,
        ...
        // Map Polls to PollResponses containig vote counts nd poll creator details
        List<Long> pollIds = polls.map(Poll::getId).getContent();
        Map<Long, Long> choiceVoteCountMap = getChoiceVoteCountMap(pollIds);
        Map<Long, Long> pollUserVoteMap = getPollUserVoteMap(currentUser, pollIds);
    ...
    public void validatePageNumberAndSize(int page, int size) {
        ...
        if (size > AppConstants.MAX_PAGE_SIZE) {
            throw new BadRequestException("Page size must not be grater than " + AppConstants.MAX_PAGE_SIZE);
        }
    }
